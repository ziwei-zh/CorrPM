...
#  from networks.ce2p_pose_edge_attention import PSPModule
...
class _new_NonLocalBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True):
        super(_new_NonLocalBlockND, self).__init__()
    ...
    def forward(self, x, x_=None):
        ...
        theta_x = theta_x.permute(0, 2, 1)
        phi_x = self.phi(x_).view(batch_size, self.inter_channels, -1)
        f = torch.matmul(theta_x, phi_x)
...
class NewNONLocalBlock2D(_new_NonLocalBlockND):
    def __init__(self, in_channels, inter_channels=None, sub_sample=True, bn_layer=True):
        super(NewNONLocalBlock2D, self).__init__(in_channels,
                                              inter_channels=inter_channels,
