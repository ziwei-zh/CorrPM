...
def compute_mean_ioU(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val', part=None):
    if part != None:
...
atr_palette = [0,0,0, 128,0,0, 255,0,0, 0,85,0, 170,0,51, 255,85,0, 0,0,85, 0,119,221, 85,85,0, 0,85,85, 85,51,0, 52,86,128, 0,128,0, 0,0,255, 51,170,221, 0,255,255, 85,255,170, 170,255,85]
def compute_mean_ioU_atr(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val', part=None, label=None, rotations=None):
    list_path = os.path.join(datadir, dataset + '_id.txt')
    val_id = [i_id.strip() for i_id in open(list_path)]
    ...
    pixel_accuracy = (tp.sum() / pos.sum()) * 100 # pixel_accuracy = overall_accuracy
    mean_accuracy = ((tp / np.maximum(1.0, pos)).mean()) * 100
    precision = tp / np.maximum(1.0, res)
    recall = tp / np.maximum(1.0, pos)
    #  set_trace()
    avg_pre = (tp / np.maximum(1.0, res)).mean()
    avg_f1 = (2 * precision * recall / (np.maximum(1.0, (precision + recall)))).mean()
    IoU_array = (tp / np.maximum(1.0, pos + res - tp))
    IoU_array = IoU_array * 100
    mean_IoU = IoU_array.mean()
...
def compute_mean_ioU_mhp(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val', dataset_list='_new.txt'):
    ...
    for i, im_name in enumerate(val_id):
        gt_path = os.path.join(datadir, dataset, 'single_parsing_anno_big', im_name + '.png')
...
def compute_mean_ioU_cihp(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val', dataset_list='_add_bkg_id.txt'):

    list_path = os.path.join(datadir, dataset, dataset + dataset_list) # test on whole image
    val_id = [i_id.strip() for i_id in open(list_path)]
...
def compute_mean_ioU_part_data(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val', part=None):
    if part != None:
        print("Using %s part to compute miou: " % part)
        list_path = os.path.join(datadir, dataset +  '_id.txt')
    else:
        list_path = os.path.join(datadir, dataset + '_part_id.txt')
    val_id = [i_id.strip() for i_id in open(list_path)]
...
def compute_mean_ioU_val_part(preds, labels, scales, centers, rotations, num_classes, datadir, input_size=[473, 473], dataset='val', part=None):
    confusion_matrix = np.zeros((num_classes, num_classes))
